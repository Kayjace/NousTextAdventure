...
export const saveOrUpdateStory = async (story: any) => {
  if (!story || typeof story !== 'object') {
    console.error("Invalid story data, not saving.");
    return;
  }
  
  // 필수 속성 검사 (예: characterBio, storyStart 등이 있는지)
  const requiredProps = ['chosenCharacter', 'chosenGenre', 'storyStart'];
  if (!requiredProps.every(prop => prop in story)) {
    console.error("Story data missing required properties, not saving.");
    return;
  }
